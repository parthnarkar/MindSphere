...
const AdminAuthPage = () => {
  ...
  const handleLogin = async (e) => {
    ...
    setError("");
    try {
      // Pre-check: try to find a users doc for this email to validate admin role
      if (email) {
        try {
          ...
          if (!snap.empty) {
            ...
            if (foundRole && foundRole !== 'admin') {
              setError(`This email is registered as '${foundRole}'. Please use the regular login for '${foundRole}' accounts.`);
              return;
            }
          } else {
            setError(`No admin account found for this email. Please contact system administrator.`);
            return;
          }
        } catch (preErr) {
      ...
      if (!signedUp || acctRole !== 'admin') {
        try { await logoutUser(); } catch (_) {}
        if (!signedUp) {
          setError(`This account has not completed admin setup. Please contact system administrator.`);
        } else {
          setError(`This account is registered as '${acctRole}'. Please use the regular login for '${acctRole}' accounts.`);
        }
        return;
      }
  ...
  const handleSignup = async (e) => {
    ...
    setError("");
    try {
      await registerUser(email, password, 'admin', { name });
      setIsLogin(true);
    } catch (err) {
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-black px-4 overflow-hidden">
      ...
      <div className="relative z-10 bg-white/70 shadow-xl rounded-2xl overflow-hidden w-full max-w-6xl grid grid-cols-1 md:grid-cols-2">
        ...
        <div className="hidden md:flex flex-col items-center justify-center p-2 bg-gradient-to-b from-white to-gray-50">
          ...
          <p className="mt-4 text-base text-[#90A4AE] text-center px-8">Administrative access to institutional mental health data and analytics.</p>
        </div>

        {/* Right panel - form */}
        <div className="p-10 md:p-16">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl md:text-3xl font-bold text-[#263238]">{isLogin ? "Admin Access" : "Create Admin Account"}</h2>
            <div className="text-sm text-[#263238]">{isLogin ? "New admin?" : "Already registered?"} <button type="button" onClick={() => { setError(""); setIsLogin(!isLogin); }} className="ml-2 px-3 py-1 rounded-md text-black font-medium cursor-pointer border-1 border-black hover:bg-[#FF8C42] hover:text-white transition focus:outline-none focus:ring-4 focus:ring-[#FF8C42]/30">{isLogin ? "Sign up" : "Login"}</button></div>
          </div>

          <p className="mt-3 text-base text-[#263238]">{isLogin ? "Sign in to access admin dashboard" : "Create new admin account"}</p>

          {/* Admin role indicator */}
          <div className="mt-6">
            <div className="flex items-center space-x-2">
              <div className="px-4 py-2 rounded-full text-base font-medium bg-[#FF8C42] text-white">
                Administrator
              </div>
            </div>
          </div>
          ...
          <form onSubmit={isLogin ? handleLogin : handleSignup} className="mt-6 space-y-4">
            {!isLogin && (
              <div>
                ...
                <input value={name} onChange={(e) => setName(e.target.value)} className="w-full px-5 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#FF8C42] focus:border-[#FF8C42] text-base" placeholder="Admin Name" required />
              </div>
            )}

            <div>
              <label className="block font-semibold text-base text-[#263238] mb-1">Email</label>
              <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="w-full px-5 py-3 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#263238] hover:border-[#FF8C42] text-base" placeholder="admin@institution.edu" required />
            </div>
